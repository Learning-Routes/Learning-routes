...
**Tree clean. Not merged, not deployed. Nothing run against production.**

Five commits, in the order the brief asked, with one deliberate departure noted in §1/§3 below.
`AdvancementPolicy`'s three outcomes are untouched.
...
|---|---|---|
| Main (`bin/rails test`) | 638 runs, 2504 assertions, 0F 0E | **671 runs, 2684 assertions, 0F 0E** |
| Browser (`bin/rails test:system`) | 50 runs, 370 assertions, 0F 0E | **50 runs, 370 assertions, 0F 0E** |
| Combined (`bin/rails test test engines/*/test`) | 1032 runs, 4095 assertions, **3F 1E** | **1065 runs, 4275 assertions, 3F 1E** |
| RuboCop | clean, 563 files | **clean, 570 files** |
...
- The header shows **no "Completar paso"** on the exam step page, the exam page, or the results
  page. It is still there on a lesson step.
